...
| N 日おき | 3 日おき |
| 毎月・日付 | 毎月 25 日 / 毎月 末日 |
| 毎月・第 n 曜日 | 毎月 第 2 月曜 / 毎月 第 2・第 4 木曜（週は複数選べる） |
| 毎年 | 毎年 4/1 |
...
このとき黒い窓は出しません。代わりに、動かしたものが画面へ出すはずだった内容を
`%APPDATA%\Koyomi\actions.log` に残します。うまく動いたかどうかは、ここで確かめられます。

引数のパスは書いたとおりに渡ります。`D:\XAMPP\htdocs\app.php` のような
逆斜線区切りも、空白を含むパスを `"` で囲んだ場合もそのままです。
引数に書いたファイルが見つからないときは、保存する前に赤字で知らせます。

なお、作業フォルダは **開くもの** に指定したプログラムの置き場所になります。
スクリプトが相対パスでファイルを読み書きする場合はご注意ください。

**設定した連動動作は、その場で試せます。**
「連動」タブの **いま試す** を押すと、アラームの時刻を待たずに動きます。
結果には渡した引数が 1 つずつ並ぶので、区切りが意図どおりかどうかも目で確かめられます。
隣の **実行の記録** は `actions.log` をそのまま開きます。
...
秒単位から年単位まで扱えます。足したその場で計り始めます。

終わったときの音は、内蔵の音のほかに **音声ファイル** やフォルダからのランダムも選べます。
音はタイマーごとに覚えるので、別々の音で鳴らし分けられます。
...
開き直したときや次に起動したときは、閉じたときの大きさと位置で出ます（最大化も含む）。
